...
const [ponyHire, setPonyHire] = useState(false);  // signup: needs to hire a pony (affects price) — off by default
  ...
  const TOURNAMENT_ENTRY = { mode: 'contact', endpoint: 'https://poloact.co.uk/api/tournament-entry', email: CONTACT_EMAIL };
  const entryContact = TOURNAMENT_ENTRY.mode === 'contact';
  // Named on the entry email so the office knows which club it came from.
  const CLUB_NAME_FOR_ENTRIES = 'PoloACT Demo Polo Club';
